...
class MappedCache:
    def __init__(self, associativity=1, cache_size=8, block_size=32, replacement_policy='lru', cache_level='l1'):
        ...
        self.cache = pd.DataFrame(index=range(int((self.cache_size * 1024) / self.block_size)), columns=range(2))
        self.cache.columns = ['Tag', 'Address']
    ...
    def process_address(self, type, input_address):
        ...
        if self.replacement_policy == 'lru':
            ...
            self.cache['Address'][index+i] = address
            return 'miss'
